...
def select_function(prompt: PromptInput, functions: List[FunctionDefinition], model) -> str:
    ...
    # On autorise deux variantes de tokenisation par nom: avec ou sans espace initial.
    ...
    while len(generated_ids) < max_len:
        matching = [(name, seq) for name, seq in candidates if _matches_prefix(seq, generated_ids)]
        if not matching:
            break

        completed = [name for name, seq in matching if len(seq) == len(generated_ids)]
        next_token_ids = {seq[len(generated_ids)] for _, seq in matching if len(seq) > len(generated_ids)}
